...
const Dashboard = () => {
  ...
  if (authIsLoading || loading) return <div className="min-h-screen flex items-center justify-center bg-gray-50"><Loader2 className="h-12 w-12 animate-spin text-empowerPurple" /> <p className="ml-4 text-lg text-gray-600">Loading...</p></div>;
  if (error) return <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4"><Navbar /><div className="text-center"><Info className="h-12 w-12 mx-auto text-red-500 mb-4" /><h2 className="text-2xl font-bold mb-2 text-red-700">Error</h2><p className="text-gray-600 mb-4">{error}</p><Button onClick={() => window.location.reload()}>Try Again</Button></div><Footer /></div>;
  if (!profile) return <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4"><Navbar /><div className="text-center"><Info className="h-12 w-12 mx-auto text-orange-500 mb-4" /><h2 className="text-2xl font-bold mb-2 text-orange-700">Profile Not Found</h2><p className="text-gray-600 mb-4">Please complete onboarding.</p><Button onClick={() => navigate("/onboarding")}>Go to Onboarding</Button></div><Footer /></div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="container px-4 py-8 flex-grow">
        {/* Header */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8"><div><h1 className="text-3xl font-bold">Welcome, {profile.first_name || 'User'}</h1><p className="text-gray-600 mt-1">Your learning journey</p></div><div className="mt-4 md:mt-0 flex items-center space-x-4"><Badge className="bg-empowerPurple capitalize">{getFieldLabel(profile)}</Badge><Badge variant="outline" className="border-empowerPurple text-empowerPurple capitalize"><Clock className="h-3 w-3 mr-1" />{getTimelineLabel(profile)} plan</Badge></div></div>
        {/* Goal Card */}
        <Card className="mb-8 shadow-md"><CardHeader className="pb-2"><CardTitle>Goal</CardTitle><CardDescription>Your target objective</CardDescription></CardHeader><CardContent><p className="text-lg font-medium">{getGoal(profile)}</p>{profile.challenges && (<p className="text-sm text-gray-500 mt-2">Challenges: {profile.challenges}</p>)}</CardContent></Card>
        {/* Tabs */}
        <Tabs defaultValue="overview" value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center"><TabsList className="grid grid-cols-3 w-full sm:w-[400px] mb-4 sm:mb-0"><TabsTrigger value="overview">Overview</TabsTrigger><TabsTrigger value="roadmap">Roadmap</TabsTrigger><TabsTrigger value="resources">Resources</TabsTrigger></TabsList><div className="flex items-center w-full sm:w-auto justify-end"><p className="text-sm text-gray-600 mr-2 whitespace-nowrap">Progress:</p><div className="w-full sm:w-48 flex items-center"><Progress value={overallProgress} className="h-2 mr-2 flex-grow" /><span className="text-sm font-medium">{overallProgress}%</span></div></div></div>

          {/* Overview Tab */}
           <TabsContent value="overview" className="animate-fade-in">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <Card className="shadow-sm"><CardHeader><CardTitle className="flex items-center"><GraduationCap className="h-5 w-5 mr-2 text-empowerPurple" />Focus</CardTitle></CardHeader><CardContent>{currentMilestoneForFocus ? (<div className="space-y-4"><h3 className="font-medium">{currentPhaseForFocus?.title}: {currentMilestoneForFocus.title}</h3><p className="text-gray-600">{currentMilestoneForFocus.description || "-"}</p><div className="flex items-center text-sm text-gray-500"><Clock className="h-4 w-4 mr-1" />Est: {currentMilestoneForFocus.estimated_time || "N/A"}</div></div>) : roadmap ? (<div className="text-center py-6"><CheckCheck className="h-10 w-10 mx-auto text-green-500 mb-2" /><p className="text-gray-600 font-medium">Roadmap Complete!</p></div>) : (<div className="text-center py-6"><Info className="h-10 w-10 mx-auto text-gray-400 mb-2" /><p className="text-gray-600">No roadmap available.</p></div>)}</CardContent><CardFooter>{roadmap && (<Button variant="outline" size="sm" className="w-full" onClick={() => setActiveTab("roadmap")}><ArrowRight className="ml-2 h-4 w-4" />View Roadmap</Button>)}</CardFooter></Card>
                    <Card className="shadow-sm"><CardHeader><CardTitle className="flex items-center"><BookOpen className="h-5 w-5 mr-2 text-empowerPurple" />Resources</CardTitle><CardDescription>Current step</CardDescription></CardHeader><CardContent>{currentResourcesForOverview.length > 0 ? (<div className="space-y-2">{currentResourcesForOverview.map((r) => (<a key={r.id} href={r.url} target="_blank" rel="noopener noreferrer" className="flex items-center p-2 hover:bg-gray-100 rounded-md group"><div className="w-10 h-10 rounded mr-3 flex-shrink-0 bg-gray-200 flex items-center justify-center">{r.imageUrl ? <img src={r.imageUrl} alt={r.title} className="w-full h-full object-cover" /> : <BookOpen className="w-5 h-5 text-gray-400"/>}</div><div className="flex-grow"><h4 className="font-medium text-sm group-hover:text-empowerPurple">{r.title}</h4><p className="text-xs text-gray-500">{r.type} • {r.platform}</p></div><ExternalLink className="h-4 w-4 text-gray-400 group-hover:text-empowerPurple ml-2" /></a>))}</div>) : (<p className="text-sm text-gray-500 text-center py-4">{currentMilestoneForFocus ? "No resources." : "-"}</p>)}</CardContent><CardFooter>{roadmap && (<Button variant="outline" size="sm" className="w-full" onClick={() => setActiveTab("resources")}><ArrowRight className="ml-2 h-4 w-4" />View All</Button>)}</CardFooter></Card>
                    <Card className="shadow-sm md:col-span-2"><CardHeader><CardTitle className="flex items-center"><Award className="h-5 w-5 mr-2 text-empowerPurple" />Profile</CardTitle></CardHeader><CardContent><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"><div className="p-4 bg-gray-100 rounded-lg"><h3 className="font-medium text-sm text-gray-500 mb-1">Background</h3><p className="font-medium capitalize">{getBackgroundLabel(profile)}</p></div><div className="p-4 bg-gray-100 rounded-lg"><h3 className="font-medium text-sm text-gray-500 mb-1">Experience</h3><p className="font-medium capitalize">{getExperienceLabel(profile)}</p></div><div className="p-4 bg-gray-100 rounded-lg"><h3 className="font-medium text-sm text-gray-500 mb-1">Commitment</h3><p className="font-medium">{profile.time_available || "-"}</p></div></div></CardContent></Card>
                </div>
           </TabsContent>
            ...
            <Card className="shadow-sm">
              ...
              <CardContent>
                {roadmap && phasesForTimeline && phasesForTimeline.length > 0 ? (
                  ...
                  <div className="text-center py-12"><Info className="h-12 w-12 mx-auto text-gray-400 mb-4" /><h3 className="text-xl font-medium mb-2">No Roadmap Yet</h3><p className="text-gray-600">Please complete onboarding or check back later.</p></div>
                )}
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="resources" className="animate-fade-in">
               <Card className="shadow-sm"><CardHeader><CardTitle>Learning Resources</CardTitle><CardDescription>Curated materials for your journey</CardDescription></CardHeader><CardContent>{roadmap && roadmap.phases && roadmap.phases.length > 0 ? ( roadmap.phases.map((phase, phaseIndex) => ( <div key={phase.id || phaseIndex} className="mb-8 last:mb-0"> <h3 className="font-semibold text-lg mb-3">{phase.title}</h3> {phase.milestones && phase.milestones.length > 0 ? ( phase.milestones.map((milestone, milestoneIndex) => ( <div key={milestone.id || milestoneIndex} className="mb-6 pl-4 border-l-2 border-gray-200"> <h4 className="font-medium text-md mb-2">{milestone.title}</h4> {milestone.resources && milestone.resources.length > 0 ? ( <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"> {milestone.resources.map((resource) => ( <ResourceCard key={resource.id} resource={{...resource, imageUrl: resource.image_url }} /> ))} </div> ) : ( <p className="text-sm text-gray-500 italic">No resources.</p> )} </div> )) ) : ( <p className="text-sm text-gray-500 italic pl-4">No milestones.</p> )} {phaseIndex < roadmap.phases.length - 1 && <Separator className="my-6" />} </div> )) ) : ( <div className="text-center py-12"><Info className="h-12 w-12 mx-auto text-gray-400 mb-4" /><h3 className="text-xl font-medium mb-2">No Resources</h3><p className="text-gray-600">Resources will appear with your roadmap.</p></div> )}</CardContent></Card>
          </TabsContent>
        </Tabs>
      </div>
      <Footer />
